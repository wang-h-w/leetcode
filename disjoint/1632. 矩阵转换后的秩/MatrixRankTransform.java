class MatrixRankTransform {
    private int[] p; //并查集，用于合并相同大小的元素，保证相同大小的元素秩相同，且应为这些相同元素中秩的最大值
    private int[] vals; //对应下标的秩的值(下标使用indexs数组中的下标值表示)
    private Integer[] indexs; //转换二维下标为一维，存储下标，并按照矩阵中的值大小排序
